    ...
    <div class="cart-tool">
      <div class="select-all">
        <input
          class="chooseAll"
          type="checkbox"
          :checked="AllCart && cartInfoList.length > 0"
          @click="allChoose"
        />
        <span>全选</span>
      </div>
      <div class="option">
        <a @click="deleteChooseCart">删除选中的商品</a>
...
export default {
  ...
  methods: {
    ...
    async changeIsChecked(cart, event) {
      ...
      try {
        await this.$store.dispatch("shopcart/changeCheckById", {
          skuId: cart.skuId,
          isChecked: checked,
        });
        this.getData();
    ...
    async allChoose(event) {
      let isChecked = event.target.checked ? "1" : "0";
      try {
        await this.$store.dispatch("shopcart/allChoose", isChecked);
        this.getData();
      } catch (error) {
        alert(error.message);
      }
    },
    async deleteChooseCart() {
      try {
        //派发一个action
        await this.$store.dispatch("shopcart/deleteChooseCart");
        this.getData();
      } catch (error) {
        alert(error.message);
      }
  ...
  computed: {
    ...
    totalPrice() {
      ...
      this.cartInfoList.forEach(
        (item) => (sum += item.cartPrice * item.skuNum)
      );
      return sum;
    },
  },
