<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      >
        <span
          ...
          @click="changeDefault(address, userAddress)"
      ...
      <ul>
        <li>
          <b
            ...
            >件商品，总商品金额</b
          >
          <span>¥{{ allSkuNum.allPrice }}.00</span>
        </li>
        <li>
          <b>返现：</b>
          <span>0.00</span>
    ...
    <div class="trade">
      <div class="price">
        应付金额:　<span>¥{{ allSkuNum.allPrice }}.00</span>
      </div>
...
export default {
  ...
  computed: {
    ...mapGetters(["detailArrayList", "tradeNo"]),
    ...mapState({
      userAddress: (state) => {
    ...
    allSkuNum() {
      let { detailArrayList } = this;
      let sum = 0;
      let allPrice = 0;
      detailArrayList.forEach((item) => {
        sum += item.skuNum;
        allPrice += item.orderPrice;
      });
      return { sum, allPrice };
    },
    userDefaultaddress() {
  ...
  methods: {
    ...
    async submitOrder() {
      let { tradeNo, userDefaultaddress, detailArrayList } = this;
      let data = {
        consignee: userDefaultaddress.consignee,
        consigneeTel: userDefaultaddress.phoneNum,
        deliveryAddress: userDefaultaddress.fullAddress,
        paymentWay: "ONLINE",
        orderComment: this.mag,
